# widgets/react-widgets agent instructions
...
React-specific Widgets adapters. Keep version-specific behavior separate; a pass for one adapter is not proof for another. The React 19 adapter (`react-19/`) is authored for Packages since 2026-09-21: manifest-first modules under `modules/base/{client,server}` and `modules/page/{client,server}`, compiled by the `ts` bundler on the development runtime and supplied by the toolchain; read [its architecture](docs/architecture.md) before changing it. The widget lifecycle and, since 2026-09-22, the page controller (`page/client`: URI, query string changes, nested layouts) are executed by the command line's `web` acceptance in a browser; server rendering of pages is not implemented on the development runtime.
